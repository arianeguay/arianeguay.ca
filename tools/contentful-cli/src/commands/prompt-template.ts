...
export function registerPromptTemplateCommand(program: Command) {
  program
    ...
    .description(
      "Generate a ChatGPT data generation prompt or Markdown template",
    )
    .requiredOption("--type <id>", "Content type ID")
    .option(
      "--locale <code>",
      "Locale code for example skeleton (e.g., en)",
      "fr",
    )
    .option(
      "--env <path>",
    ...
    .action(async (opts) => {
      ...
      for (const field of ct.fields) {
        ...
        switch (type) {
          ...
          case "RichText":
            example.fields[key] = {
              [loc]: { nodeType: "document", data: {}, content: [] },
            };
            break;
          case "Array":
            if (
          ...
          case "Link":
            if (
              (field as any).linkType === "Asset" ||
              (field as any).validations?.some((v: any) => v.linkMimetypeGroup)
            ) {
              example.fields[key] = {
                [loc]: {
