...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const pdfBuf = (await instance.toBuffer()) as Buffer;

    // Optional S3 upload
    let pdfUrl: string | undefined;
    const bucket = process.env.AWS_S3_BUCKET;
    const region = process.env.AWS_REGION || 'us-east-1';
    const hasS3 = !!(process.env.AWS_ACCESS_KEY_ID && process.env.AWS_SECRET_ACCESS_KEY && bucket);
    const key = `invoices/Invoice-${invoice.number}-${invoice.id}.pdf`;

    if (hasS3) {
      const s3 = new S3Client({ region });
      await s3.send(
        new PutObjectCommand({ Bucket: bucket!, Key: key, Body: pdfBuf, ContentType: 'application/pdf' })
      );
    ...
    return Response.json(withRelations(memDb.invoices.find((i) => i.id === invoice.id) || inv));
  } catch (e: any) {
    return Response.json({ error: e?.message ?? 'unexpected' }, { status: 500 });
  }
}
